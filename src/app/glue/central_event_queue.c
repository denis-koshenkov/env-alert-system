...
typedef enum EventId {
    ...
    EVENT_ID_VOID_CB_WITH_UINT8_AND_USER_DATA_PARAMS,
} EventId;
...
typedef struct VoidCbWithUint8AndUserDataParamsEvent {
    Event event;
    CentralEventQueueVoidCbWithUint8AndUserDataParams cb;
    uint8_t param_uint8;
    void *user_data;
} VoidCbWithUint8AndUserDataParamsEvent;
...
enum {
    CENTRAL_EVENT_QUEUE_MAX_MESSAGE_SIZE =
        MAX7(sizeof(Event), sizeof(NewTemperatureSampleEvent), sizeof(NewPressureSampleEvent),
             ...
             sizeof(VoidCbWithUint8AndUserDataParamsEvent))
};
...
/**
 * @brief Handle "void cb with uint8 and user data params" event by invoking the callback.
 *
 * @param event "Void cb with uint8 and user data params" event.
 */
static void
handle_void_cb_with_uint8_and_user_data_params_event(const VoidCbWithUint8AndUserDataParamsEvent *const event)
{
    EAS_ASSERT(event);
    EAS_ASSERT(event->cb);
    event->cb(event->param_uint8, event->user_data);
}
...
static void central_event_queue_thread_run()
{
    ...
    while (1) {
        ...
        case EVENT_ID_VOID_CB_WITH_UINT8_AND_USER_DATA_PARAMS: {
            EAS_ASSERT(message_size == sizeof(VoidCbWithUint8AndUserDataParamsEvent));
            const VoidCbWithUint8AndUserDataParamsEvent *const event =
                (const VoidCbWithUint8AndUserDataParamsEvent *const)message;
            handle_void_cb_with_uint8_and_user_data_params_event(event);
            break;
        }
        default:
...
{
    EAS_ASSERT(cb);
    VoidCbWithUint8AndUserDataParamsEvent event = {
        .event.id = EVENT_ID_VOID_CB_WITH_UINT8_AND_USER_DATA_PARAMS,
        .cb = cb,
        .param_uint8 = param_uint8,
        .user_data = user_data,
    };
    push_event_to_queue((const uint8_t *const)&event, sizeof(VoidCbWithUint8AndUserDataParamsEvent));
}
