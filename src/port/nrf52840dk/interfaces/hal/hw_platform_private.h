#ifndef ENV_ALERT_SYSTEM_SRC_PORT_NRF52840DK_INTERFACES_HAL_HW_PLATFORM_PRIVATE_H
#define ENV_ALERT_SYSTEM_SRC_PORT_NRF52840DK_INTERFACES_HAL_HW_PLATFORM_PRIVATE_H
 ...
 * p_twim_inst.
 * 4. I2C transaction is complete. TWIM ISR handler accesses cb and user_data fields to submit this data to the central
 * event queue, so that cb is executed from the central event queue thread context.
 * 5. Central event queue processes the event and executes cb with user_data as a parameter.
 */
typedef struct {
    /** @brief Callback that should be executed when the I2C transaction is complete.
     *
     * Volatile because it is set inside sht3x_driver_i2c_write/read (central event queue context). This variable is
     * read inside the NRF TWIM ISR (ISR context) in order to submit it to the event queue.
     */
    volatile SHT3X_I2CTransactionCompleteCb cb;
...
#endif /* ENV_ALERT_SYSTEM_SRC_PORT_NRF52840DK_INTERFACES_HAL_HW_PLATFORM_PRIVATE_H */
