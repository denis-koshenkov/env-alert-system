...
{
    /* Give cb and user_data to the test by populating the provided pointers, so that the test can simulate calling the
     * TransceiverReceiveCb */
    TransceiverReceiveCb *receive_cb_pointer = (TransceiverReceiveCb *)mock().getData("receiveCb").getPointerValue();
    void **receive_cb_user_data_pointer = (void **)mock().getData("receiveCbUserData").getPointerValue();
    *receive_cb_pointer = cb;
    *receive_cb_user_data_pointer = user_data;
