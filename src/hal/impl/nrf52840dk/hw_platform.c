...
static void send_button_state()
{
    ...
    EAS_LOG_INF("Sending button state %d", (int)button_state);
    my_lbs_send_button_state_indicate(button_state);
}

static void button_timer_expired_cb(void *user_data)
{
    send_button_state();
}
...
#define BUTTON_TIMER_PERIOD_MS 5000
...
static void hw_platform_init_part_12(void *user_data)
{
    ...
    button_timer = eas_timer_create(BUTTON_TIMER_PERIOD_MS, EAS_TIMER_PERIODIC, button_timer_expired_cb, NULL);
    eas_timer_start(button_timer);
